<!-- This is the modal -->
<div id="connect-to-us" class="uk-modal connect-to-us">
    <a class="uk-modal-close uk-close"></a>
    <div class="uk-modal-dialog small">
        <p class="title-h3 uk-modal-title">Связаться с нами</p>
        <form action="#">
            <fieldset class="custom-input">
                <label for="f-1">Номер телефона</label>
                <input type="tel" class="tel-input" id="f-1" placeholder="(___) ___ - __ - __">
            </fieldset>
            <fieldset class="custom-input">
                <label for="f-2">Ваше имя</label>
                <input type="text" id="f-2" placeholder="Введите ваше имя">
            </fieldset>
            <fieldset>
                <input type="submit" value="Отправить" class="btn btn-big btn-accent btn-hover-light">
            </fieldset>
        </form>
    </div>
</div>

<footer class="page-footer">
    <div class="container">
        <div class="uk-grid uk-grid-small uk-grid-width-1-1 uk-grid-width-medium-1-3 uk-grid-width-large-1-3">
            <div>
                <p class="footer-title">Каталог товаров</p>
                <ul class="footer-link-list">
                    <li><a href="#">Для спорта</a></li>
                    <li><a href="#">Для туризма</a></li>
                    <li><a href="#">Для рыбалки</a></li>
                    <li><a href="#">Для охоты</a></li>
                    <li><a href="#">Для охраны</a></li>
                    <li><a href="#">Для спецназа</a></li>
                    <li><a href="#">Для веломана</a></li>
                    <li><a href="#">Для города</a></li>
                </ul>
            </div>
            <div class="footer-padding-left">
                <p class="footer-title">Бренд M-tac</p>
                <ul class="footer-link-list">
                    <li><a href="#">Store Locator</a></li>
                    <li><a href="#">О бренде</a></li>
                    <li><a href="#">Технологии</a></li>
                    <li><a href="#">Обзоры</a></li>
                    <li><a href="#">Новости</a></li>
                    <li><a href="#">Сотрудничество</a></li>
                    <li><a href="#">Контакты</a></li>
                </ul>
            </div>
            <div class="footer-padding-left">
                <p class="footer-title">Наши контакты</p>
                <ul class="footer-tel-list">
                    <li><a href="tel:+380443467990">+38 (044) 346-79-90</a></li>
                    <li><a href="tel:+380676786789">+38 (067) 678-67-89</a></li>
                    <li><a href="tel:+380934583556">+38 (093) 458-35-56</a></li>
                </ul>
                <div class="footer-connection-block">
                    <b>E-mail: </b><a href="mailto:info@m-tac.ua">info@m-tac.ua</a>
                </div>
                <div class="footer-connection-block">
                    <b>Адрес: </b><address>03040, Украина, г. Киев, ул. Шевченка 23</address>
                </div>
                <ul class="social-list">
                    <li><a href="#"><i class="iconmoon icon-fb" aria-hidden="true"></i></a></li>
                </ul>
            </div>
        </div>
        <div class="footer-copyright-block">
            <span>&copy; 2017, M-Tac, Military Tactical</span>
        </div>
    </div>
</footer>

<div id="scroll-up">
    <span></span>
</div>


